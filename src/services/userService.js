...
export async function list({ page = 1, page_size = 20, active, role }) {
    const where = []
    const params = []
    let i = 1
    if (active !== undefined) { where.push(`active = $${i++}`); params.push(!!active) }
    if (role) { where.push(`role = $${i++}::user_role`); params.push(role) }
...
export async function createUser({ nome, email, senha, role = "user", telefones = [] }) {
    ...
    try {
       ...
       returning id, nome, email, active, role, created_at, updated_at`,
        ...
        if (Array.isArray(telefones) && telefones.length) {
            ...
            const placeholders = telefones.map((_, idx) =>
...
export async function addPhone({ usuario_id, ddd, numero, is_whatsapp = false }) {
    const { rows } = await pool.query(
...
export async function login({ email, senha }) {
    ...
    return { id: user.id, nome: user.nome, email: user.email, role: user.role, active: user.active }
}
