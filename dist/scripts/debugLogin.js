...
async function debugLogin() {
    try {
        ...
        const { rows: testRows } = await index_1.pool.query("SELECT NOW() as agora");
        console.log("✅ Conexão OK! Hora do servidor:", testRows[0].agora);
        // 2. Buscar usuário no banco
        const { rows } = await index_1.pool.query("SELECT * FROM usuarios WHERE email = $1", [email]);
        if (rows.length === 0) {
            console.log("❌ Usuário não encontrado no banco!");
            // Mostrar todos os usuários para debug
            const { rows: allUsers } = await index_1.pool.query("SELECT email, nome, role FROM usuarios LIMIT 5");
        ...
        for (const senha of testPasswords) {
            try {
                const isValid = await (0, password_1.verifyPassword)(senha, user.senha);
