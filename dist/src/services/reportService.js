...
async function getMonthlyRevenue(year) {
    const query = `
        ...
        ORDER BY mes
    `;
    const { rows } = await index_1.pool.query(query, [year]);
...
async function getTopServices(year) {
    let query = `
        SELECT
            s.id,
            s.title as nome,
...
async function getGeneralStats(year) {
    ...
    const revenueQuery = `
        ...
        WHERE EXTRACT(YEAR FROM a.data) = $1
            AND a.status = 'finalizado'
    `;
    const { rows: revenueRows } = await index_1.pool.query(revenueQuery, [year]);
    ...
    `;
    const { rows: appointmentRows } = await index_1.pool.query(appointmentsQuery, [year]);
    ...
    const clientsQuery = `
        ...
        WHERE EXTRACT(YEAR FROM data) = $1
    `;
    const { rows: clientRows } = await index_1.pool.query(clientsQuery, [year]);
    // Ticket médio
    ...
    `;
    const { rows: avgTicketRows } = await index_1.pool.query(avgTicketQuery, [year]);
    return {
