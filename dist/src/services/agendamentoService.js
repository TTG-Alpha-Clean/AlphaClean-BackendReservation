...
exports.getDailySlots = getDailySlots;
exports.list = list;
exports.getByIdWithClientInfo = getByIdWithClientInfo;
...
async function countAtSlot({ data, horario }) {
    ...
    `;
    const { rows } = await index_1.pool.query(q, [data, horario]);
    return rows[0]?.total || 0;
}
async function findById(id) {
    const { rows } = await index_1.pool.query(`SELECT * FROM agendamentos WHERE id = $1`, [id]);
    return rows[0] || null;
}
function assertOwnershipOrAdmin(ag, user) {
    if (!ag)
        throw new apiError_1.default(404, "Agendamento não encontrado");
    if (user.role !== "admin" && ag.usuario_id !== user.id) {
        throw new apiError_1.default(403, "Você não tem permissão para acessar este agendamento");
    }
}
// ===== API PÚBLICA =====
async function getDailySlots({ data }) {
    const { rows } = await index_1.pool.query(`SELECT horario::text, COUNT(*) FILTER (WHERE status IN ('agendado','finalizado'))::int AS ocupados
         FROM agendamentos
...
async function list(filters) {
    const { status, data_ini, data_fim, usuario_id, page = 1, page_size = 20, isAdmin = false } = filters;
    const where = [];
    const params = [];
    let i = 1;
    if (usuario_id) {
    ...
    const [dataResult, countResult] = await Promise.all([
        index_1.pool.query(baseQuery, params),
...
async function getByIdWithClientInfo(id, user) {
...
async function deleteAgendamento(id, user) {
    ...
    if (!canDelete) {
        const allowedStatuses = user.role === "admin"
            ...
            : "cancelados";
        throw new apiError_1.default(400, `Só é possível excluir agendamentos ${allowedStatuses}`);
    }
    ...
    `;
    const { rows } = await index_1.pool.query(deleteQuery, [id]);
    if (!rows[0]) {
        throw new apiError_1.default(404, "Agendamento não encontrado");
    }
    return {
...
async function create(payload) {
    const { usuario_id, modelo_veiculo, cor, placa, servico_id, data, horario, observacoes = null } = payload;
    const plate = sanitizePlate(placa);
    if (isPastDateTime(data, horario)) {
        throw new apiError_1.default(400, "Não é possível agendar no passado");
    }
    const validSlot = buildSlotsOfDay().includes(horario);
    if (!validSlot)
        throw new apiError_1.default(400, "Horário fora do expediente ou inválido para o slot configurado");
    ...
    const dupQ = `
        ...
        LIMIT 1
    `;
    const { rowCount: dup } = await index_1.pool.query(dupQ, [usuario_id, data, horario]);
    if (dup)
        throw new apiError_1.default(409, "Você já possui um agendamento ativo neste horário");
    const plateCheckQ = `
        SELECT 1 FROM agendamentos
        ...
        LIMIT 1
    `;
    const { rowCount: plateExists } = await index_1.pool.query(plateCheckQ, [plate, data]);
    if (plateExists)
        throw new apiError_1.default(409, "Esta placa já possui agendamento neste dia");
    const ins = `
        ...
        RETURNING *
    `;
    const { rows } = await index_1.pool.query(ins, [
        usuario_id, modelo_veiculo, cor, plate,
        servico_id, data, horario, observacoes
    ]);
    return rows[0];
}
async function reschedule(id, user, { data, horario }) {
    const ag = await findById(id);
    assertOwnershipOrAdmin(ag, user);
    if (ag.status !== "agendado") {
        throw new apiError_1.default(400, "Só é possível reagendar agendamentos com status 'agendado'");
    }
    if (isPastDateTime(data, horario)) {
        throw new apiError_1.default(400, "Não é possível reagendar para o passado");
    }
    const validSlot = buildSlotsOfDay().includes(horario);
    if (!validSlot)
        throw new apiError_1.default(400, "Horário fora do expediente");
    ...
    const conflictQ = `
        ...
        WHERE data = $1 AND horario = $2::time AND id != $3
          AND status IN ('agendado','finalizado')
    `;
    const { rows: conflictRows } = await index_1.pool.query(conflictQ, [data, horario, id]);
    const conflicts = conflictRows[0]?.total || 0;
    if (conflicts >= SCHEDULE.MAX_CONCURRENT) {
        throw new apiError_1.default(409, "Horário não disponível");
    }
    const upd = `
        UPDATE agendamentos
        ...
        RETURNING *
    `;
    const { rows } = await index_1.pool.query(upd, [data, horario, id]);
    return rows[0];
}
async function updateStatus(id, user, newStatus) {
    ...
    if (!canTransition(ag.status, newStatus, user.role)) {
        ...
        throw new apiError_1.default(400, `Transição de '${ag.status}' para '${newStatus}' não permitida. ${transitions}.`);
    }
    const upd = `
        UPDATE agendamentos
        ...
        RETURNING *
    `;
    const { rows } = await index_1.pool.query(upd, [newStatus, id]);
    return rows[0];
}
async function updateAgendamento(id, user, payload) {
    const ag = await findById(id);
    assertOwnershipOrAdmin(ag, user);
    if (ag.status !== "agendado") {
        throw new apiError_1.default(400, "Só é possível editar agendamentos com status 'agendado'");
    }
    const { modelo_veiculo, cor, placa, servico_id, data, horario, observacoes } = payload;
    if (!modelo_veiculo || !placa || !servico_id || !data || !horario) {
        throw new apiError_1.default(400, "Campos obrigatórios: modelo_veiculo, placa, servico_id, data, horario");
    }
    const plate = sanitizePlate(placa);
    if (isPastDateTime(data, horario)) {
        throw new apiError_1.default(400, "Não é possível agendar no passado");
    ...
    const upd = `
        ...
        RETURNING *
    `;
    const { rows } = await index_1.pool.query(upd, [
